...
export function RightSidebar({ userId, readingListIds, onRemoveFromList, onAddToList, velocity }: Props) {
  ...
  // Fetch reading list when tab is 'list' or readingListIds changes
  useEffect(() => {
    if (tab !== 'list') return;
    setListLoading(true);
  ...
  // Fetch hot papers once
  useEffect(() => {
    if (tab !== 'hot' || hotPapers.length > 0) return;
    setHotLoading(true);
    api.hotPapers(8).then(d => setHotPapers(d.papers)).catch(() => {}).finally(() => setHotLoading(false));
  }, [tab]);
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 pt-4 pb-3 border-b border-gray-100 shrink-0">
        <button onClick={() => setVelOpen(!velOpen)} className="flex items-center justify-between w-full">
          ...
          <div className="flex items-center gap-2">
            {velocity ? (
              <>
                <span className="text-xs font-bold text-gray-800 font-mono">{velocity.latest}/wk</span>
                <span className={`text-[10px] font-mono font-medium ${velocity.delta >= 0 ? 'text-emerald-500' : 'text-red-500'}`}>{velocity.delta >= 0 ? '+' : ''}{velocity.delta}</span>
              </>
        ...
        {velOpen && vData.length > 0 && (
          <div className="mt-3">
            <div className="flex items-end gap-[2px] h-12">
              {vData.map((v, i) => (
                <div key={i} className="flex-1" title={`${v.period_start}: ${v.count}`}>
                  <div className="w-full rounded-t" style={{ height: `${(v.count / maxV) * 100}%`, backgroundColor: i === vData.length - 1 ? '#3b82f6' : '#cbd5e1' }} />
                </div>
              ))}
            </div>
            <div className="flex justify-between mt-1 text-[8px] text-gray-400 font-mono">
              <span>{vData[0]?.period_start?.slice(5)}</span>
              <span>avg {velocity?.avg}</span>
              <span>{vData[vData.length - 1]?.period_start?.slice(5)}</span>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="flex border-b border-gray-100 shrink-0">
        <TBtn active={tab === 'list'} onClick={() => setTab('list')} icon={<BookOpen size={12} />} label={`List (${readingListIds.size})`} />
        <TBtn active={tab === 'hot'} onClick={() => setTab('hot')} icon={<Flame size={12} />} label="Hot" />
        <TBtn active={tab === 'recs'} onClick={() => setTab('recs')} icon={<Lightbulb size={12} />} label="For You" />
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        {tab === 'list' && (
          <div className="p-3 space-y-1">
            {listLoading ? <CenterLoader /> : listPapers.length === 0 ? (
              <Empty icon={<BookOpen className="w-8 h-8 text-gray-300 mb-2" />} text="Empty reading list" sub="Search papers to add" />
            ) : listPapers.map(p => (
              <PCard key={p.id} paper={p} onAction={() => onRemoveFromList(p.id)} actionIcon={<X size={11} />} />
            ))}
          </div>
        )}
        {tab === 'hot' && (
          <div className="p-3 space-y-1">
            ...
            {hotLoading ? <CenterLoader /> : hotPapers.map(p => (
              <PCard key={p.id} paper={p}
        ...
        {tab === 'recs' && (
          <div className="p-3 space-y-1">
            ...
            {recsLoading ? <CenterLoader /> : recs.length === 0 ? (
              <Empty icon={<Lightbulb className="w-8 h-8 text-gray-300 mb-2" />} text="No recommendations" sub="Add papers to get suggestions" />
            ) : recs.map(p => (
              <PCard key={p.id} paper={p}
                reason={(p as Recommendation).reason}
                onAction={() => readingListIds.has(p.id) ? onRemoveFromList(p.id) : onAddToList(p.id)}
                actionIcon={readingListIds.has(p.id) ? <X size={11} /> : <BookOpen size={11} />} />
            ))}
          </div>
...
function TBtn({ active, onClick, icon, label }: { active: boolean; onClick: () => void; icon: React.ReactNode; label: string }) {
  return (
...
function PCard({ paper, onAction, actionIcon, highlight, reason }: { paper: Paper; onAction: () => void; actionIcon: React.ReactNode; highlight?: boolean; reason?: string }) {
      ...
      <div className="flex items-start gap-1.5">
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1.5 mt-1">
            {paper.citation_count != null && <span className="text-[9px] text-gray-400">{fmtCit(paper.citation_count)}</span>}
            {paper.update_date && <span className="text-[9px] text-gray-400">{String(paper.update_date).slice(0, 4)}</span>}
          </div>
...
function CenterLoader() { return <div className="py-8 flex justify-center"><Loader2 size={18} className="animate-spin text-gray-400" /></div>; }
function Empty({ icon, text, sub }: { icon: React.ReactNode; text: string; sub: string }) {
  return <div className="flex flex-col items-center py-12 text-center">{icon}<p className="text-sm text-gray-400">{text}</p><p className="text-xs text-gray-300 mt-1">{sub}</p></div>;
}
