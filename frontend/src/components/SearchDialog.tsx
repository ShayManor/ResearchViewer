...
export function SearchDialog({ onClose, onSelectPaper, onAddToList, readingList }: Props) {
  ...
  const doSearch = useCallback((q: string) => {
    if (!q.trim() && !fSubject) { setPapers([]); setAuthors([]); return; }
    setLoading(true);
    if (tab === 'papers') {
      ...
      if (!q.trim()) { setAuthors([]); setLoading(false); return; }
      api.searchAuthors(q, 15).then(d => setAuthors(d.authors)).catch(() => setAuthors([])).finally(() => setLoading(false));
    }
  }, [tab, fSubject, fStart, fEnd, fSort, fOrder]);

  useEffect(() => { clearTimeout(debRef.current); debRef.current = setTimeout(() => doSearch(query), 300); }, [query, doSearch]);
  useEffect(() => { if (query || fSubject) { clearTimeout(debRef.current); debRef.current = setTimeout(() => doSearch(query), 100); } }, [fSubject, fStart, fEnd, fSort, fOrder]);

  const hasFilters = fSubject || fStart || fEnd;
      ...
      <div className="fixed inset-0 flex items-start justify-center pt-[10vh] px-4">
        <div className="w-full max-w-2xl bg-white border border-gray-200 rounded-2xl shadow-2xl overflow-hidden animate-fade-in-up" onClick={e => e.stopPropagation()}>
          ...
          <div className="flex gap-1 px-5 py-2 bg-gray-50/50 border-b border-gray-100">
            <button onClick={() => { setTab('papers'); setPapers([]); setAuthors([]); }} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${tab === 'papers' ? 'bg-white text-gray-800 shadow-sm border border-gray-200' : 'text-gray-400 hover:text-gray-500'}`}><FileText size={12} /> Papers</button>
            <button onClick={() => { setTab('authors'); setPapers([]); setAuthors([]); }} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${tab === 'authors' ? 'bg-white text-gray-800 shadow-sm border border-gray-200' : 'text-gray-400 hover:text-gray-500'}`}><User size={12} /> Authors</button>
          </div>
          ...
          <div className="max-h-[45vh] overflow-y-auto">
            ...
            {tab === 'authors' && (
              <>
                {!query && <div className="px-5 py-8 text-center"><p className="text-sm text-gray-400">Search authors by name</p></div>}
                {query && !authors.length && !loading && <div className="px-5 py-8 text-center"><p className="text-sm text-gray-400">No authors found</p></div>}
                {authors.map(a => (
                  <div key={a.author_id} className="px-5 py-3 hover:bg-gray-50 border-b border-gray-50 transition-colors">
